...
import SVMDetailsPage from './algorithms/SVM/SVMDetailsPage';
import RandomForestDetailsPage from './algorithms/RandomForest/RandomForestDetailsPage';
import DecisionTreeDetailsPage from './algorithms/DecisionTree/DecisionTreeDetailsPage';

export const ForestryMLApp = () => {
  const [activeTab, setActiveTab] = useState('overview');
  const [showingSVMDetails, setShowingSVMDetails] = useState(false);
  const [showingRandomForestDetails, setShowingRandomForestDetails] = useState(false);
  const [showingDecisionTreeDetails, setShowingDecisionTreeDetails] = useState(false);
  ...
  // Add a handler for navigation that resets all detail views
  const handleNavigation = (tab: string) => {
    setActiveTab(tab);
    setShowingSVMDetails(false);
    setShowingRandomForestDetails(false);
    setShowingDecisionTreeDetails(false);
  };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <nav className="bg-green-800 text-white p-4 rounded-lg mb-6">
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={() => handleNavigation('decision-trees')}
            className={`flex items-center space-x-2 px-4 py-2 rounded-md ${
          ...
          <button
            onClick={() => handleNavigation('random-forest')}
            className={`flex items-center space-x-2 px-4 py-2 rounded-md ${
          ...
          <button
            onClick={() => handleNavigation('neural-network')}
            className={`flex items-center space-x-2 px-4 py-2 rounded-md ${
          ...
          <button
            onClick={() => handleNavigation('gradient-boosting')}
            className={`flex items-center space-x-2 px-4 py-2 rounded-md ${
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        {showingSVMDetails ? (
          <SVMDetailsPage 
            onBack={() => setShowingSVMDetails(false)}
            onRandomForest={() => {
              setShowingSVMDetails(false);
              setShowingRandomForestDetails(true);
              setActiveTab('random-forest');
            }}
          />
        ) : showingRandomForestDetails ? (
          <RandomForestDetailsPage 
            onBack={() => setShowingRandomForestDetails(false)}
          />
        ) : showingDecisionTreeDetails ? (
          <DecisionTreeDetailsPage 
            onBack={() => setShowingDecisionTreeDetails(false)}
          />
        ) : (
          <>
            {activeTab === 'overview' && (
            ...
            {activeTab === 'decision-trees' && (
              <DecisionTreeAnalysis 
                onLearnMore={() => setShowingDecisionTreeDetails(true)} 
              />
            )}

            {activeTab === 'random-forest' && (
              <RandomForestAnalysis 
                onLearnMore={() => setShowingRandomForestDetails(true)} 
              />
            )}

            {activeTab === 'svm' && (
              <SVMAnalysis 
                onLearnMore={() => setShowingSVMDetails(true)} 
              />
            )}
